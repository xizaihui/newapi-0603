	...
	apiRouter.Use(middleware.GlobalAPIRateLimit())
	{
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			adminRoute.Use(middleware.AdminAuth())
			{
				...
				// Phase 2: per-user group ratio override (单用户级倍率覆盖)
				adminRoute.GET("/:id/group_ratios", controller.GetUserGroupRatios)
				adminRoute.PUT("/:id/group_ratios", controller.SetUserGroupRatios)
				adminRoute.DELETE("/:id/group_ratios/:group", controller.DeleteUserGroupRatio)
				// Phase 3: per-user model price override (单用户级模型按次价格覆盖)
				adminRoute.GET("/:id/model_prices", controller.GetUserModelPrices)
				adminRoute.PUT("/:id/model_prices", controller.SetUserModelPrices)
				adminRoute.DELETE("/:id/model_prices/:model", controller.DeleteUserModelPrice)
